...
class Loading_screen():
    def __init__(self):
        ...
        self.number_frame_walking = GOBLIN_DICT["NUMBER_FRAME_WALKING"]
        self.image_walking = []
        for i in range(1,self.number_frame_walking+1):
                self.image_walking.append(pygame.image.load(GOBLIN_DICT["WALKING_IMAGE_PATH"]+str(i).zfill(3)+".png").convert_alpha())  
                self.image_walking[i-1] = pygame.transform.scale(self.image_walking[i-1],vec(self.image_walking[i-1].get_size())*RESIZE_COEFF)
        self.anim_total_time_w = GOBLIN_DICT["ANIMATION_WALKING_TOTAL_TIME"]  # in ms
